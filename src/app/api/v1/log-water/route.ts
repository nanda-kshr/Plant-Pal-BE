...
export async function GET(req: NextRequest){
    ...
    const logs = await db.collection("water_logs").find({user_id, plant_id: new ObjectId(plant_id)}).sort({ loggedAt: -1 }).skip(skip || 0).limit(limit || 10).toArray();
...
export async function POST(req: NextRequest){
    ...
    const { plant_id, water, wateredAt } = body;
